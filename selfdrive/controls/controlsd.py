...
from openpilot.selfdrive.controls.lib.customcontrol import CustomControl
...
class Controls:
  def __init__(self, CI=None):
    ...
    self.VM = VehicleModel(self.CP)
        ...
        # initialising the custom control
    self.custom_control = CustomControl(self.CP)
    self.customControlEngaged = False
  ...
  def mabxii_control(self, CS):
    """Functions similarly to state_control, but returns a CarControl packet with MABXII control"""
    ...
    CC.longActive = (self.enabled and not self.events.contains(ET.OVERRIDE_LONGITUDINAL)
                     and (self.CP.openpilotLongitudinalControl or self.custom_profile_enabled))

    actuators = CC.actuators

    # start the profile if able to do so
    if self.enabled and not self.custom_control.isRunning and not self.customControlEngaged:
      self.custom_control.start()
      self.customControlEngaged = True
    elif not self.enabled:
      if self.custom_control.isRunning:
        self.custom_control.stop()
      self.customControlEngaged = False

    # longitudinal control state should be off/stopping/starting/pid
    actuators.longControlState = self.custom_control.long_control_state

    # setting acceleration
    pid_accel_limits = self.CI.get_pid_accel_limits(self.CP, CS.vEgo, self.v_cruise_helper.v_cruise_kph * CV.KPH_TO_MS)
    actuators.accel = self.custom_control.update(CC.longActive, CS, pid_accel_limits)
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    # update the custom control enabled state
    controlsState.customProfileEnabled = self.custom_profile_enabled
    controlsState.profileStartTime = self.custom_control.start_time
    controlsState.profileCurrentTime = self.custom_control.current_time
    controlsState.profilePlan = ""  # Not used with MABXII control
    controlsState.profileStage = self.custom_control.current_stage
    controlsState.profileRunning = self.custom_control.isRunning
    controlsState.profileActualAccel = self.custom_control.last_output_accel
    controlsState.profileHistory = str(self.custom_control.history)
    controlsState.mabxiiActive = self.custom_control.mabxii_active
    controlsState.targetAccel = self.custom_control.target_accel
  ...
  def params_thread(self, evt):
    while not evt.is_set():
      self.is_metric = self.params.get_bool("IsMetric")
      self.custom_profile_enabled = self.params.get_bool("MabxiiControlEnabledToggle")
